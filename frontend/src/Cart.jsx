...
const Cart = () => {
    const { cart, removeFromCart, ingredientCounts, clearCart, user } =
        useUser();
        ...
        try {
            ...
            if (!inventoryResponse.ok) {
                const insufficientItems =
                    inventoryData.insufficient_items || [];
            ...
            const orderResponse = await fetch(
                "http://127.0.0.1:5000/active_orders",
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (orderResponse.ok) {
                ...
                alert(
                    `Failed to add order to active orders. Reason: ${orderResult.message}`
                );
            }
        } catch (error) {
